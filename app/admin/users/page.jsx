      ...
      <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-ruby-900 leading-tight">Admin Users</h1>
              <p className="text-[11px] text-gray-500">{data?.users?.length ?? 0} admins · you are {data?.myRole || '—'}</p>
            </div>
          </div>
          <nav className="flex items-center gap-1 text-sm">
            <Link href="/admin" className="px-3 py-1.5 rounded-lg hover:bg-gray-100 text-gray-600">Submissions</Link>
            <span className="px-3 py-1.5 rounded-lg bg-ruby-50 text-ruby-800 font-medium">Users</span>
          </nav>
        </div>
      </header>
