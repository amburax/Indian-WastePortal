...
export default function ForgotPassword() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setDone(d.message || 'If that email has an account, we’ve sent a reset link.');
    } catch { setDone('If that email has an account, we’ve sent a reset link.'); }
    ...
    <div className="min-h-screen bg-mesh flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md glass-frosted rounded-2xl p-8">
          ...
          <h1 className="font-display text-2xl font-bold text-slate-800">Reset your password</h1>
          <p className="text-sm text-slate-500 mt-1 mb-6">Enter your account email and we’ll send you a reset link.</p>
          ...
          {done ? (
            ...
            <form onSubmit={submit} className="space-y-4">
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@company.in" autoComplete="email"
          ...
          <p className="text-xs text-slate-400 mt-6"><Link href="/login" className="text-ruby-800 underline">Back to sign in</Link></p>
        </div>
      </main>
    </div>
  );
