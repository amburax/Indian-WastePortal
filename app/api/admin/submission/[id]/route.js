...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const payment = await db.get('SELECT status, amount_paise, razorpay_payment_id, paid_at, refund_id, refund_amount_paise, refund_status, refunded_at FROM payments WHERE org_id = ? ORDER BY created_at DESC LIMIT 1', [id]);
