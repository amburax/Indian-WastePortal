 ...
 *   EMAIL_FROM:     e.g. 'Indian Waste Portal <no-reply@indianwasteportal.in>'
...
export async function sendEmail({ to, subject, html, text }) {
  ...
  const from = process.env.EMAIL_FROM || 'Indian Waste Portal <no-reply@indianwasteportal.in>';
...
export function receiptEmail({ orgName, authPerson, category, token }) {
  ...
  const trackUrl = token
    ? `${process.env.APP_BASE_URL || 'https://indianwasteportal.in'}/status/${token}`
    : null;
...
const APP = () => process.env.APP_BASE_URL || 'https://indianwasteportal.in';
