...
/**
 * Authoritative role lookup (the token carries only the email — role can change
 * after login, so it's always read fresh from the DB). Returns 'superadmin' |
 * 'admin'. Defaults to 'admin' (least privilege) if the row is missing.
 */
export async function getAdminRole(db, email) {
