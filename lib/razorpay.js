...
/**
 * Refund a captured payment (real) or a synthetic dev refund. `amount` is in
 * paise (omit / full = whole payment). Returns { id, status, amount }.
 */
export async function refundPayment({ paymentId, amount, notes }) {
  if (!isRealRazorpay()) {
    return { id: `rfnd_dev_${randomUUID().slice(0, 12)}`, status: 'processed', amount, _dev: true };
  }
  const rzp = await client();
